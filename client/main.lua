...
local function updateRadialMenu(type)
    if currentType == type then return end 
    
    if currentType == "park" then
        lib.removeRadialItem('park_system')
    elseif currentType == "list" then
        lib.removeRadialItem('park_system_list')
    end
    ...
    elseif type == "list" then
        lib.addRadialItem({
            ...
            icon = 'list',
            onSelect = function()
                TriggerEvent('parking:client:checkVehicleList')
            end
        })
    end
end

local function refreshMenuOnStart()
...
if Config.EnableParkCommand then
    ...
    RegisterCommand('parklish', function()
        TriggerEvent('parking:client:checkVehicleList')
    end, false)
end
...
RegisterNetEvent('parking:client:createtarget', function(netId)
    ...
    exports.ox_target:addLocalEntity(veh, {
        ...
        },
        {
            ...
            onSelect = function(data)
                TriggerEvent("parking:client:checkVehicleStatus", {
                    entity = data.entity
                })
            end
        }
...
RegisterNetEvent('parking:client:checkVehicleStatus', function(data)
    ...
    QBCore.Functions.TriggerCallback('parking:server:checkOwnership', function(isOwner)
        QBCore.Functions.TriggerCallback('parking:server:getDepotPrice', function(depotPrice)
            ...
            local fuel = Entity(vehicle).state.fuel or 0
...
CreateThread(function()
    local cfg = Config.Impound.Citizen
    if not cfg.enabled then return end

    RequestModel(cfg.model)
    while not HasModelLoaded(cfg.model) do Wait(0) end

    local impoundPed = CreatePed(4, cfg.model, cfg.coords.x, cfg.coords.y, cfg.coords.z , cfg.coords.w, false, false)
    ...
    exports.ox_target:addLocalEntity(impoundPed, {
        {
            ...
            onSelect = function()
                TriggerEvent('parking:client:openImpoundMenu')
            end
        },
    })
end)
...
CreateThread(function()
    local cfg = Config.Impound.Officer
    if not cfg.enabled then return end

    RequestModel(cfg.model)
    while not HasModelLoaded(cfg.model) do Wait(0) end

    local officerPed = CreatePed(4, cfg.model, cfg.coords.x, cfg.coords.y, cfg.coords.z, cfg.coords.w, false, false)
    ...
    exports.ox_target:addLocalEntity(officerPed, {
        {
            icon = cfg.icon,
    ...
    for _, v in ipairs(vehicles) do
        table.insert(options, {
            ...
            onSelect = function()
                ...
                if alert == 'confirm' then
                    TriggerServerEvent('parking:server:releaseVehicleByOfficer', v.plate, Config.Impound.SpawnCoords)
                end
            end
        })
    end
...
RegisterNetEvent('parking:client:notify', function(text, type, timeout)
...
AddEventHandler('onResourceStart', function(resource)
    ...
    Wait(1000)
    refreshMenuOnStart()
    print('^2[Parking System]^7 Radial Menu has been initialized.')
end)
